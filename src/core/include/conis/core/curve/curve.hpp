...
public:
    ...
    void setCoords(const std::vector<Vector2DD> &verts) { vertices_ = verts; }
    void setNormals(const std::vector<Vector2DD> &normals) { normals_ = normals; }

    void setVertex(int idx, const Vector2DD &coord);
